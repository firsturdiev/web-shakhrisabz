export default defineEventHandler(async (event) => {
  const apiToken = ''; // replace with your actual API token
  const response = await fetch('http://207.154.250.85:4011/api/galleries?pagination[pageSize]=100&populate=*', {
    headers: {
      'Authorization': `Bearer ${apiToken}`
    }
  });

  if (!response.ok) {
    console.error('Failed to fetch galleries:', response.status, response.statusText);
    return;
  }

  const galleries = await response.json();
  return galleries.data.map((gallery) => ({ photo: 'http://207.154.250.85:4011' + gallery.attributes.photo.data.attributes.url }));
});