export default defineEventHandler(async (event) => {
  ...
  const response = await fetch('http://207.154.250.85:4011/api/projects?pagination[pageSize]=100&populate=*', {
    headers: {
  ...
  return projects.data.map((projectsItem) => ({ 
    ...
    photo: 'http://207.154.250.85:4011' + projectsItem.attributes.photo.data.attributes.url
   }));
});
